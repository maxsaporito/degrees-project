...
def shortest_path(source, target):
    ...
    frontier = deque([(source, [])])  # The queue stores tuples of (current_person_id, path_to_current_person)
    explored = set()
    ...
    while frontier:
        ...
        for movie_id, person_id in neighbors_for_person(current_person):
            if person_id not in explored and not any(person_id == p for _, p in frontier):
